...
- **Layout**: 左侧尺寸线框住英雄跨度 0.51→0.29；右侧目标刻度 0.20 与残差 0.09 用标题红引线标出，不用卡片墙。
- **Title**: 浸没式光刻缺陷指标回收
...
- **Layout**: 左主仪表 D0 0.51→0.29；右指标树沿竖向标尺展开 D0/capture/nuisance/SNV/PWP/kill/adder，完成项暗红、监控项浅线。
...
- **Layout**: 横向工程瀑布，从峰值 0.51 起跳；已关闭段暗红，预测段标题红虚线，残差 0.09 用阻塞红收尾。
