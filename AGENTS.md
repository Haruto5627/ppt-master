...
- Extract-only reading of Word / PowerPoint / Excel / PDF (no edit, no PPT generation) uses [`skills/document-extract/SKILL.md`](skills/document-extract/SKILL.md): MarkItDown by default, Docling for scanned or layout-heavy PDFs. Do not substitute it for Generate source intake (`source_to_md.py`).
