...
- Extract-only reading of Word / PowerPoint / Excel / PDF (no edit, no PPT generation) uses [`skills/document-extract/SKILL.md`](skills/document-extract/SKILL.md): MarkItDown by default, Docling for scanned or layout-heavy PDFs; dump Word/PPT images into a sibling `images/` folder via `extract_office_images.py` (Word also writes `<stem>.captions.json`). Long Word rewrite → Markdown + Pandoc HTML is section 0 of that skill; output names follow the rewritten content. Do not substitute it for Generate source intake (`source_to_md.py`).
