...
- Extract-only reading of Word / PowerPoint / Excel / PDF (no edit, no PPT generation) uses [`skills/document-extract/SKILL.md`](skills/document-extract/SKILL.md): MarkItDown by default, Docling for scanned or layout-heavy PDFs; dump Word/PPT images into a sibling `images/` folder via `extract_office_images.py` (multiple Word files in one directory each get `images/<stem>/`; Word also writes a captions JSON). Do not substitute it for Generate source intake (`source_to_md.py`).
- Rewriting a long Word file into human-readable Markdown uses [`skills/word-rewrite/SKILL.md`](skills/word-rewrite/SKILL.md): Pandoc for LaTeX, stdlib image/caption extract, HTML `<table>` for merged cells; deliver `{slug}.md` only and do not export HTML.
